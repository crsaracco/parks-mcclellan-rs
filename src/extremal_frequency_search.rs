    ...
    if deviation < (nut as f64) * (err as f64) {
        ...
        'loop_13: loop {
            ...
            if ell > *klow {
                ...
                } else if *extremal_frequencies_changed {
                ...
                if *extremal_frequencies_changed {
                    update_loop_variables(j-1, ell-1, klow, extremal_frequencies_changed, extremal_frequencies);
                    return; // ON TO THE NEXT EXTREMA
                }
                else {
    ...
    *comp = (nut as f64) * (err as f64);
    ...
    for k in (ell+1)..kup {
        ...
        if (nut as f64) * (err as f64) - *comp <= 0.0 {
            ...
            update_loop_variables(j-1, k-1, klow, extremal_frequencies_changed, extremal_frequencies);
            local_max_found = true;
    ...
    if !local_max_found {
        update_loop_variables(j-1, kup-1, klow, extremal_frequencies_changed, extremal_frequencies);
    }
}
