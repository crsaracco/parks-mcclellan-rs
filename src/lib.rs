...
#[allow(dead_code)]
pub enum FilterType {
    /// A multi-band filter (low-pass, high-pass, band-pass, notch, etc).
    MultipleBand,

    /// A digital [Differentiator](https://en.wikipedia.org/wiki/Differentiator) filter.
    Differentiator,

    /// A [Hilbert Transform filter](https://ccrma.stanford.edu/~jos/mdft/Analytic_Signals_Hilbert_Transform.html).
    HilbertTransform,
}
...
pub struct ParksMcclellanOutput {
    ...
    ///
    /// TODO: remove, since you can get this with `impulse_response.len()`.
    pub filter_length: usize,

    /// The type of the filter generated.
    ///
    /// TODO: remove, since this should be equal to the input.
    ...
    /// of the filter.
    ///
    /// TODO: remove, since most people probably don't care about this output (?)
    pub extremal_frequencies: Vec<f32>,
}

/// Design a mutliple-band filter (low-pass, high-pass, band-pass, notch, etc).
///
...
///    accurate filter. Must be greater than 0.
...
/// Design a filter using the Parks-McClellan algorithm.
///
///  - `filter_length` is the length of the impulse response of the resulting filter.
///    Must be in range [3, 128].
///  - `filter_type` is the type of filter to produce.
///  - `bands` holds information about the desired characteristics of the frequency response bands
///    for the filter. Must be in range [1, 10].
///  - `grid_density` is the "density" of the "dense grid". A higher number here will produce a more
///    accurate filter. Must be greater than 0.
...
fn print_filter_outputs(parks_mcclellan_output: &ParksMcclellanOutput) {
    match parks_mcclellan_output.filter_type {
        FilterType::MultipleBand => println!("BANDPASS FILTER"),
        FilterType::Differentiator => println!("DIFFERENTIATOR"),
        FilterType::HilbertTransform => println!("HILBERT TRANSFORMER"),
    }
    ...
    for k in 0..parks_mcclellan_output.lower_band_edges.len() {
        ...
        match parks_mcclellan_output.filter_type {
            FilterType::MultipleBand => println!("    desired value: {}", parks_mcclellan_output.desired_values[k]),
            FilterType::Differentiator => println!("    desired slope: {}", parks_mcclellan_output.desired_values[k]),
            FilterType::HilbertTransform => println!("    desired value: {}", parks_mcclellan_output.desired_values[k]),
        }
        println!("    weighting: {}", parks_mcclellan_output.weightings[k]);
        println!("    deviation: {}", parks_mcclellan_output.deviations[k]);
        match parks_mcclellan_output.filter_type {
            FilterType::MultipleBand => println!("    deviation in dB: {}", parks_mcclellan_output.deviation_dbs[k]),
            _ => {}
        }
    }
